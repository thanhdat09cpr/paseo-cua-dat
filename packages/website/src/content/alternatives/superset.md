...
description: Paseo is an AGPL-3.0 Superset alternative that runs locally without an account and supports application plugins across its server and clients.
nav: Superset
order: 51
---
...
Paseo orchestrates coding agents from desktop, mobile, web, and the CLI. Open source under AGPL-3.0.
...
Paseo runs locally without an account, is licensed under AGPL-3.0, provides structured interfaces for supported agent harnesses, and supports application plugins with server-side behavior and native client components.

## License

Paseo is open source under AGPL-3.0. You can audit, fork, modify, and redistribute it.
