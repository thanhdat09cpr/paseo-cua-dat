...
export class Session {
  ...
  private subscribeToPluginChanges(
    ...
    return pluginRuntime.subscribe((pluginId) => {
      this.emit({
        type: "status",
        payload: { status: "plugin_catalog_changed", pluginId },
      });
    });
  }
        ...
        snapshot = await this.agentManager.resumeAgentFromPersistence(
          ...
          matched
            ? {
                ...
                roleBinding: matched.record.launchContract ? undefined : matched.record.roleBinding,
                launchContract: matched.record.launchContract,
                launchProfile: matched.record.launchProfile,
              }
            : undefined,
        );
      } catch (error) {
  ...
  private async handleSendAgentMessageRequest(
    ...
    try {
      ...
      let dispatchResult: {
        disposition: "out_of_band" | "steered" | "turn_started";
      };
      try {
