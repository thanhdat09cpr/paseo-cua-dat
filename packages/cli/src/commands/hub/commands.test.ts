...
describe("Hub commands", () => {
  ...
  it("keeps permission inspection read-only but rejects agent-initiated grants", async () => {
    const daemon = new FakeDaemon("https://hub.test");
    daemon.returnedPermissions = ["hub.execute"];
    const dependencies = {
      daemon: new FakeDaemonConnection(daemon),
    ...
    await assert.rejects(runHubPermissionChange("grant", "hub.execute", {}, dependencies), {
