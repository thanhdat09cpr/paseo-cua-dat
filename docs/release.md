...
to the personal ad-hoc lane: it builds Apple Silicon `arm64`, creates a DMG,
updater ZIP/blockmaps, SHA-256 and source metadata, skips Apple notarization, and keeps the release
...
Personal ad-hoc releases publish a verified `latest-mac.yml` from the fork only.
CI checks the exact version, ARM64 ZIP, SHA-512, size and blockmap, uploads all
assets first, uploads the channel manifest last, then publishes the draft as a
prerelease. An installed fork build can therefore use **Check for updates** and
**Update** to download, install and restart into a newer `-paseo.N` build. Manual
checks bypass staged rollout admission; automatic checks retain the configured
`rolloutHours`. The updater continues to keep arbitrary prerelease versions out
